...
class Model(object):
	def __init__(self, filename):
		...
		self.originOffset = glm.vec3(self.boundsCenter.x, self.boundsCenter.y, self.boundsCenter.z)
	...
	def _compute_bounds(self):
		# Compute bounding information to allow consistent placement/scaling
		xs = [v[0] for v in self.objFile.vertices]
		ys = [v[1] for v in self.objFile.vertices]
		zs = [v[2] for v in self.objFile.vertices]

		min_vec = glm.vec3(min(xs), min(ys), min(zs))
		max_vec = glm.vec3(max(xs), max(ys), max(zs))

		self.boundsMin = min_vec
		self.boundsMax = max_vec
		self.boundsSize = max_vec - min_vec
		self.boundsHalfExtent = self.boundsSize * 0.5
		self.boundsCenter = (min_vec + max_vec) * 0.5
	...
	def GetModelMatrix(self):
		...
		centerMat = glm.translate(identity, -self.originOffset)

		return translateMat * rotationMat * scaleMat * centerMat
	...
	def Render(self):
		...
		# DEBUG: draw vertex points first to confirm vertex data is bound and visible
		# This will render small points at each vertex; if these are visible but triangles
		# are not, the issue is likely triangle rasterization or shader-based discard.
		try:
			from OpenGL.GL import glPointSize, glDrawArrays, GL_POINTS
			glPointSize(2.0)
			glDrawArrays(GL_POINTS, 0, self.vertexCount)
		except Exception:
			pass

		# Then render triangles normally
